#include <LEDStrip.h>

#define SIPIN 9
#define DIPIN 3
#define CLKPIN 4
#define LATCHPIN 5

LEDStrip mystrip(DIPIN,SIPIN,LATCHPIN,CLKPIN);

#define DELAY    0
#define SPULSES  4

void setup()
{
}  

void dosPulse()
{
  uint8_t i;

  for (i=0; i < SPULSES; i++) {
    mystrip.sPulse();
  } 
}

void docolor(uint8_t red, uint8_t green, uint8_t blue)
{
  docolor(red, green,blue, (128/SPULSES) - 1, 1);
}

void docolor(uint8_t red, uint8_t green, uint8_t blue, uint16_t n, uint8_t speed)
{
  uint8_t i;

  if (speed == 1)
    mystrip.rgbPush(red,green,blue);
  else
    mystrip.rgbPush2X(red,green,blue);
    
  mystrip.latch();
  dosPulse();

  for (i=0; i< n; i++) {
    mystrip.blankPush();
    mystrip.latch();
    dosPulse();
    //delay(DELAY);
  }
}

long count = 0 ;

void loop()
{
  int i;
  static int mode=0;
  count ++ ;
  if( count == 100 ) {
    mode++;
    if( mode > 7 ) mode =0;
    count = 0; 
  }
  
  switch(mode ) {
    case 0 :
      docolor(UP,OFF,OFF,128/(1*SPULSES),1);
      break;
    case 1 :
      docolor(UP,OFF,DOWN,128/(2*SPULSES),2);
      break;
    case 2 :
      docolor(OFF,ON,OFF,128/(2*SPULSES),2);
      break;
    case 3 :
      docolor(OFF,OFF,ON,128/(2*SPULSES),2);
      break;
    case 4 :
      docolor(OFF,ON,ON,128/(2*SPULSES),2);
      break;
    case 5 :
      docolor(ON,ON,ON,128/(2*SPULSES),2);
      break;
    case 6 :
      docolor(ON,OFF,ON,128/(2*SPULSES),2);
      break;
    case 7 :
      docolor(ON,OFF,ON,128/(2*SPULSES),2);
      docolor(ON,ON,ON,128/(2*SPULSES),2);
      docolor(ON,OFF,OFF,128/(2*SPULSES),2);
      docolor(ON,OFF,OFF,128/(2*SPULSES),2);
      break;
    default:

    docolor(UP,OFF,OFF,128/(2*SPULSES),2);
    docolor(ON,OFF,OFF,128/(2*SPULSES),1); 
    docolor(ON,UP,OFF,128/(2*SPULSES),2);
    docolor(ON,ON,OFF,128/(4*SPULSES),1);
    docolor(DOWN,ON,OFF,128/(2*SPULSES),2);    
    docolor(OFF,ON,OFF,128/(2*SPULSES),1); 
    docolor(OFF,ON,UP,128/(2*SPULSES),2);
    docolor(OFF,ON,ON,128/(4*SPULSES),1);
    docolor(OFF,DOWN,ON,128/(2*SPULSES),2);
    docolor(OFF,OFF,ON,128/(2*SPULSES),1); 
    docolor(OFF,OFF,DOWN,128/(2*SPULSES),2);
    docolor(OFF,OFF,OFF,128/(4*SPULSES),1); 
    break;
  }
  
  switch ( rand()%10) { 
    case 0:
      for(i = 0 ; i < 4 ; i++ ) {
             docolor(OFF,OFF,ON,1,1);
      }
      break;
    case 1:
      for(i = 0 ; i < 4 ; i++ ) {
             docolor(OFF,ON,OFF,1,1);
      }
      break;
      
    case 3:
      for(i = 0 ; i < 4 ; i++ ) {
             docolor(ON,OFF,OFF,128/(2*SPULSES),2);
      }
      break;
    case 4:
      for(i = 0 ; i < 20 ; i++ ) {
             docolor(rand(),rand(),rand(),rand()%10,rand());
      }
      break;      
  }
  
}

