...
@echo off
	...
	@ECHO  oscillator through the ISP interface with JTAGICE mkII
	...
	@ECHO  - USB port used for the JTAGICE mkII
	@ECHO  - jtagiceii.exe -h / AVR Studio help for more options
	@ECHO  
	@ECHO  $Name$
	@ECHO  $Revision: 3900 $                            
	@ECHO  $RCSfile$
...
@SET CPU=ATmega168
@SET TOOL="C:\Program Files (x86)\Atmel\AVR Tools\JTAGICEmkII\JTAGICEII.exe"
@SET CAL_FUSES=0x99e4
@SET CUSTOMERCODE="test.hex"


%TOOL% -cUSB -I125000 -d %CPU% -mi -f %CAL_FUSES% -e -pf -if "..\rc_calib.hex"
...
%TOOL% -cUSB -I125000 -d %CPU% -mi -ae0,0 -ve -ie "0xFF_byte.hex"
...
%TOOL% -cUSB -I125000 -d %CPU% -mi -e -pf -vf -if%CUSTOMERCODE% -Z0 -Sf0x01FF
