...
// motor speed pulses
unsigned int DELAY_X1 =(150*2); // 148 lowest (maybe 150)
unsigned int DELAY_X2 =(150*2);
unsigned  int DELAY_Y1 =(208*2); //200 lowest
unsigned  int DELAY_Y2 =(208*2);
...
//    if( !vacuum) vacuum( !digitalRead( T_VAC ) );
//    if( !head) head( !digitalRead( T_HEAD ) );
...
// not counted as limit switches  
  ...
  // any of the limit switches are reached, machine is no longer homed!
  if( limit ) {
    homed = false;
  }
...
// convery x to y to pulses
char gotoxy(int x,int y)
{
  long numXPulses=0,numYPulses=0;

  numXPulses = mmtopulses( x );
  numYPulses = mmtopulses( y );
}
