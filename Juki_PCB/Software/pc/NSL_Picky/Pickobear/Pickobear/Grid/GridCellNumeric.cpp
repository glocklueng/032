...
Flags(dwFlags)
{
    ...
    LPTSTR Iptr = sInitText.GetBuffer(sInitText.GetLength() + 1);
...
void CInPlaceNumEdit::OnTimer(UINT_PTR)
{
    Sel = GetSel();
}
...
m_nNumber(0.0)
{
	...
	// these have to be global, linker should coagulate them all into one.
	static const TCHAR DecimalSep[] = _T(".");
	static const TCHAR ThousandSep[] = _T("");

	// 0 = 123456789.0
	m_DefaultNumberFmt.Grouping = 0;

	// number of leading 0's for ( 1 = 0.1 ) , ( 0 = .1 ) but won't show anything for 0 !

	ret = ::GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_ILZERO, szBuf, __ARRAYSIZE(szBuf));
	if( ret) {
		m_DefaultNumberFmt.LeadingZero =  _tcstol(szBuf, NULL, 10);
	} else { 
		m_DefaultNumberFmt.LeadingZero =  1;

	}

	// char for seperator 123.00 
	m_DefaultNumberFmt.lpDecimalSep = (TCHAR *)&DecimalSep[0];

	// char for seperator 1,234 , we're using none since its grouping = 0
	m_DefaultNumberFmt.lpThousandSep = (TCHAR *)&DecimalSep[0];

	// LOCALE_INEGNUMBER

    ret = ::GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_INEGNUMBER, szBuf, __ARRAYSIZE(szBuf));
    if ( ret ) {
    
		m_DefaultNumberFmt.NegativeOrder =  _tcstol(szBuf, NULL, 10);

	} else {

		m_DefaultNumberFmt.NegativeOrder =  0;

	}

	/// after . (eg 2 = 1234.56 )
	m_DefaultNumberFmt.NumDigits = 2;

	SetNumberFmt( &m_DefaultNumberFmt );
...
BOOL CGridCellNumeric::Format(CString &TheString)
{
    ...
    for(int i = str.GetLength() - 1; i >= 0; --i)
    {
        ...
        TCHAR *ptr = (TCHAR*)_tcschr(digits, str[i]);
        if (ptr == NULL)
            str.Delete(i);
        else
        {
    ...
    if (*EndPtr == NULL)
    {
        ...
        if (count)
        {
            if ((m_dwFlags & TypeMask) == Integer)
            {
				...
				// didn't find a cDecimal, so lets do it another way.
				// For the swedish bug
				if( pos == -1 ) { 
					int i = Formatted.GetLength();
					while( i-- ) {
						if( !isdigit( Formatted[i]  ) ) {
							cDecimal = Formatted[i];
							pos = Formatted.Find(cDecimal,0);
							break;
						}
					}
