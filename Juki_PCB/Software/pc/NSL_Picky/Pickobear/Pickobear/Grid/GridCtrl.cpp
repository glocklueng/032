...
{
    return FALSE;    // Don't erase the background.
}
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
    ...
    case MOUSE_SELECT_ALL:
        OnEditSelectAll(); // SH mod to enable mouse select all cells for editing
//      SelectAllCells(); Original
        break;
...
int  CGridCtrl::GetVisibleColumnCount()
{
    long lVisibleWidth = 0;
    ...
    return lVisibleWidth;
}

int  CGridCtrl::GetVisibleRowCount()
{
    long lVisibleWidth = 0;
...
{
	if ( nRow >= m_nRows ) return;
	if ( nCol >= m_nCols ) return;

    CString strCurrentText = GetItemText(nRow, nCol);
    if (GetSelectedCount() > 1 || strCurrentText != str)
    {
