	...
	*pdwWritten = 0;
	...
	if (!::WriteFile(m_hFile,pData,iLen,pdwWritten,lpOverlapped))
	{
		...
		/*
			...
			do {
				b = HasOverlappedIoCompleted(lpOverlapped);
				if(b==false) {
					SleepEx(0,TRUE);
					wait -- ;
					if( wait == 0 ) {
...
#define MAX_SERIAL_PORT_NAME				(256)

#define SUCCESS 1

static LONG EnumerateSerialPorts (CString &deviceName, DWORD maxLen, DWORD index)
{
	TCHAR RegPath[MAX_PATH] = _T("HARDWARE\\DEVICEMAP\\SERIALCOMM");
	HKEY hKey;
	...
	retValue = RegEnumValue (hKey, index, valueName,
		...
		NULL,
		&dwType,
		(BYTE *)deviceName.GetBuffer( MAX_SERIAL_PORT_NAME ),
		&cbData);
...
// ----------------------------- FillSerialCBox -----------------------------
// the list box contains the strings that describe each serial com port available
