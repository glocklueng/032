...
#define	FCLK_SLOW()	SPIE.CTRL = 0x53    // Set clock at 250kHz (Must be 100k-400k)
#define	FCLK_FAST()	SPIE.CTRL = 0xD0;	// Set clock at 16MHz (depends on the CSD)
...
static BYTE wait_ready (void) {
	...
	Timer2 = 16;	/* Wait for ready in timeout of 500ms */
	rcvr_spi();
	do
		res = rcvr_spi();
...
static BOOL rcvr_datablock (
	...
	Timer1 = 4;
	do {				/* Wait for data packet in timeout of approx 100mS to 150mS */
		token = rcvr_spi();
...
/* This function is called every 31.25mS                        */
/*-----------------------------------------------------------------------*/
ISR(TCE1_OVF_vect) {
	static BYTE pv=0;
	BYTE n, s;

	n = Timer1;						// 32Hz decrement timer
	if (n) Timer1 = --n;
	n = Timer2;
	if (n) Timer2 = --n;

	n = pv;
	pv = (PORTE.IN & SD_CD) + (PORTF.IN & SD_WP);	// Sample socket switch
	if (n == pv) {					                // Have contacts stabled?
		s = Stat;

		if (pv & SD_WP)	s |= STA_PROTECT;		    // WP is H (write protected)
		else s &= ~STA_PROTECT;						// WP is L (write enabled)

		if (pv & SD_CD)	s |= (STA_NODISK | STA_NOINIT);	// INS = H (Socket empty)
		else s &= ~STA_NODISK;						    // INS = L (Card inserted)

		Stat = s;
	}
}

// Interrupt occurs when the SD card is inserted
ISR(PORTE_INT0_vect) {
    uint8_t i,res;
    _delay_ms(40);
    if(!testbit(PORTE.IN,SDCD)) {
        Stat &= ~STA_NODISK;
        ...
        if(res&STA_NOINIT) {    // Failed to initialize SD
            ONRED();
            sound(250);
            OFFRED();
        }
    ...
    setbit(PORTE.INTFLAGS,PORT_INT0IF_bp);  // Clear interrupt flag
                                            // especially if bouncing caused a 2nd interrupt
}
