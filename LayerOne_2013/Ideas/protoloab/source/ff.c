...
static FRESULT sync (	/* FR_OK: successful, FR_DISK_ERR: failed */
	...
	if (res == FR_OK) {
		...
		if (disk_ioctl(CTRL_SYNC, (void*)NULL) != RES_OK)
			res = FR_DISK_ERR;
	}
	return res;
}
...
/*-----------------------------------------------------------------------*/
static DWORD get_cluster (	/* 0xFFFFFFFF:Disk error, 1:Interal error, Else:Cluster status */
	FATFS *fs,		/* File system object */
...
static BOOL test_lfn (			/* TRUE:Matched, FALSE:Not matched */
...
/*-----------------------------------------------------------------------*/
static FRESULT auto_mount (	/* FR_OK(0): successful, !=0: any error occured */
...
FRESULT f_mkfs (
	...
	if (disk_ioctl(GET_SECTOR_COUNT, &n_part) != RES_OK || n_part < MIN_SECTOR)
	...
	if (disk_ioctl(GET_SECTOR_SIZE, &SS(fs)) != RES_OK
		|| SS(fs) > _MAX_SS
		|| SS(fs) > allocsize)
		return FR_MKFS_ABORTED;
#endif
	...
	if (disk_ioctl(GET_BLOCK_SIZE, &n) != RES_OK) return FR_MKFS_ABORTED;
	n = (b_data + n - 1) & ~(n - 1);
	...
	if (!partition) {
		...
		if (disk_write(fs->win, 0, 1) != RES_OK)
			return FR_DISK_ERR;
	}
	...
	if (disk_write(tbl, b_part+0, 1) != RES_OK)
		return FR_DISK_ERR;
	if (fmt == FS_FAT32)
		disk_write(tbl, b_part+6, 1);
	...
	for (m = 0; m < N_FATS; m++) {
		...
		if (disk_write(tbl, b_fat++, 1) != RES_OK)
			return FR_DISK_ERR;
	...
	if (fmt == FS_FAT32) {
		...
		disk_write(tbl, b_part+1, 1);
		disk_write(tbl, b_part+7, 1);
	}

	return (disk_ioctl(CTRL_SYNC, (void*)NULL) == RES_OK) ? FR_OK : FR_DISK_ERR;
}
