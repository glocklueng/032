        ...
        if(testbit(Key, userinput)) {
            ...
            switch(Menu) {
                ...
                case MCH1:     // Channel 1 menu
                    setbit(Misc, redraw);                
                    if(testbit(Key,KA)) togglebit(CH1ctrl,chon);    // Channel 1 on/off
                ...
                case MCH2:                     // Channel 2 menu
                    setbit(Misc, redraw);                
                    if(testbit(Key,KA)) togglebit(CH2ctrl,chon);    // Channel 2 on/off
                    if(testbit(Key,KB)) {    // Less gain
                        if(M.CH2gain) {
                            M.CH2gain--;
                            i=0; do {   // resize
                ...
                case MMAIN5:     // Menu Select 5: Miscellaneous
                    if(testbit(Key,KA)) Menu=MCURSOR1;       // Cursors
...
/*                case MPROTOCOL2:                     // Protocol Decoding
                ...
                case MCH1HC2:     // H Cursor 2
                    ...
                    if(testbit(Key,KB)) M.Hcursor1B++;
                    if(testbit(Key,KC)) if(M.Hcursor1B) M.Hcursor1B--;
                break;
                ...
                case MCH2HC2:     // H Cursor 2
                    ...
                    if(testbit(Key,KB)) M.Hcursor2B++;
                    if(testbit(Key,KC)) if(M.Hcursor2B) M.Hcursor2B--;
                break;
            }  // end switch(menu)
...
// Display info (Menu, Grid, cursors, reference, settings...)
        ...
        if(testbit(Misc,lowbatt)) {
            memcpy_P(display_buffer,  &BATTICON, 8);   // Low battery icon
        }
...
/*                        case MAWG7:
		...
		if(testbit(MStatus, stop)) tiny_printp(0,7, PSTR("STOP"));
...
static inline void ShowCursorH(void) {
    ...
    if(!testbit(MFFT, fftmode)) {
        ...
        for(i=1; i<=126; i+=4) {
            if(testbit(MFFT,xymode)) {
                if(i<64) set_pixel(HcursorA,i);  // Vertical Line for HcursorA
            }
        ...
        if(testbit(MFFT,xymode)) {
            adjustedA=HcursorA-64;
            adjustedB=64-HcursorB-(M.HPos/2);
            tiny_printp(78,5, PSTR("X "));          // X
            printV(adjustedA*256, M.CH1gain);
            if(M.CH1gain>=4) Hcursorunit = unitmV;
            else Hcursorunit = unitV;
            lcd_putsp(Hcursorunit);
            tiny_printp(78,6, menustxt[35]+32);     // Y
