...
void setup2()                    // run once, when the sketch starts
{
     frame1.HardwareInit();   // Call this once to init the hardware. 
                              // (Only needed once, even if you've got lots of frames.)
   frame1.Clear();
...
{ 
  // What we're doing here is just switching between frames-- can be used for real animation.
	frame1.RefreshAll(150); //Draw frame buffer 1
	frame2.RefreshAll(150); //Draw frame buffer 2
	frame3.RefreshAll(150); //Draw frame buffer 3
	frame4.RefreshAll(150); //Draw frame buffer 4
}
void sleep()
{
