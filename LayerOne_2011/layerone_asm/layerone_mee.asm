...
;	James Grahame 	- ddred and extended the AVRsynth code to MeeBlip hardware.
;
;-------------------------------------------------------------------------------------------------------------------
;
;	Port Mapping
...
;; IRQ 0 -7
		    ...
		    jmp	IRQ_NONE	            ; SPI,STC Transfer complete

		    jmp	UART_RXC	            ; UART, RX COMPLETE
		    jmp	IRQ_NONE	            ; UART,UDRE data register empty
...
; Write the 8 bit sample to the R2R
; reverse bits for backwards R2R on prototype boards ( cockup in the laying out a board dept, achievement unlocked)
;-------------------------------------------------------------------------------------------------------------------
;


;write sample (R31/R30) to R2R:
; todo : tidy up!

		    ;subi	R31, 128		    	; U2 --> PB

			;andi		R31,0x7f				; mask off top 4 bits

; flicker LED's using 16 bit signed value (not very power efficient since some values  will conflict)
			out		DDRA,R31
			out		PORTA,R30
		;value to reverse for protoype board with switch r2r
			mov		r16,ZH


			; shift r31:r30:to r30 0x0ff0 becomes 0x00ff in R30
			; the meeblip DAC is 16 bit signed, ours is 8 bit
			lsr		zh
			ror		zl
			lsr		zh
			ror		zl
...
; uncomment one of these to do a simple switch test
;			lds		r16,MODEFLAGS2
;			lds		r16,MODEFLAGS2

; uncomment this to do a simple ADC test , ADC0..7
			;lds		r16,ADC_0
			;ldi r16,0xff
			

			ldi		ZL, low(reversebits<<1)	; AVR's are odd with their two byte memory
			ldi		ZH, high(reversebits<<1)

			add		ZL,r16					; add on the value to reverse, index into the lookup table

  			adc 	ZH,zero					; add carry if it exists

		   	lpm								; lookup and store in r0 (Z->R0)
     	
			; R0 = flipped value from ZL/R30


			out	    PORTC, R0	        ; output most significant byte(reversed for proto)
...
; MIDI channel will be set:
			...
			lds	    R16, SWITCH3
;			ldi	    R16, 1				; For LayerOne, always midi zero
